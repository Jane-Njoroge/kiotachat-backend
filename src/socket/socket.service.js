    ...
    socket.on(
      ...
      async ({ content, to, from, conversationId }) => {
        try {
          ...
          // Determine if the message is a file (contains img or a tag)
          const isFileMessage = /<(img|a)\b[^>]*>/.test(content);
          const fileUrl = isFileMessage ? extractFileUrl(content) : null;
          ...
          const message = await prisma.message.create({
            data: {
              ...
              senderId: fromId,
              conversationId: conversation.id,
              messageType: isFileMessage ? "file" : "text",
              fileUrl, // Store file URL if applicable
            },
            include: {
              sender: {
                select: { id: true, fullName: true, email: true, role: true },
    ...
    socket.on(
      ...
      async ({ messageId, content, from, conversationId, to }) => {
        try {
          ...
          // Determine if the updated message is a file
          const isFileMessage = /<(img|a)\b[^>]*>/.test(content);
          const fileUrl = isFileMessage ? extractFileUrl(content) : null;
...
// Helper function to extract file URL from content
const extractFileUrl = (content) => {
  const imgMatch = content.match(/<img[^>]+src=["']([^"']+)["']/);
  const aMatch = content.match(/<a[^>]+href=["']([^"']+)["']/);
  return imgMatch ? imgMatch[1] : aMatch ? aMatch[1] : null;
};

...
const userController = {
  ...
  async verifyOtp(req, res) {
    ...
    try {
      ...
      res
        .status(error.status || 400)
        .json({ message: error.message || "Invalid OTP" });
    }
  },
  ...
  async markConversationAsRead(req, res) {
    try {
      ...
      if (!id)
        throw Object.assign(new Error("Conversation ID is required"), {
          status: 400,
        });
      const conversationId = parseInt(id, 10);
      if (isNaN(conversationId))
        throw Object.assign(new Error("Invalid conversationId"), {
          status: 400,
        });
      const userId = parseInt(
        req.userId || req.body.userId || req.headers["x-user-id"],
      ...
      if (
        ...
        throw Object.assign(
          new Error("Unauthorized to mark this conversation as read"),
          { status: 403 }
        );
      }
      await prisma.conversation.update({

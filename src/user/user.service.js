...
const userService = {
  async registerUser({ fullName, email, phoneNumber, password }) {
    if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)) {
      throw Object.assign(new Error("We need a valid email address."), {
        status: 400,
      });
    }
    if (!/^\+?\d{4,15}$/.test(phoneNumber)) {
      throw Object.assign(
        new Error("Your phone number needs to be 4-15 digits."),
        { status: 400 }
      );
    }
    if (!password || /\s/.test(password)) {
      throw Object.assign(
        new Error("Your password can’t be empty or have spaces."),
        { status: 400 }
      );
    }
    ...
    if (existingUser) {
      throw Object.assign(
        new Error("This email’s already taken. Please login."),
        { status: 409 }
      );
    }
  ...
  async getAdmins(excludeUserId) {
    const parsedExcludeUserId = parseInt(excludeUserId, 10);
    if (isNaN(parsedExcludeUserId)) {
      throw Object.assign(new Error("Valid excludeUserId is required"), {
        status: 400,
      });
    }
    const admins = await userRepository.getAllUsers("ADMIN");
  ...
  async createConversation(participant1Id, participant2Id) {
    ...
    if (
      ...
      throw Object.assign(
        new Error("Valid and distinct participant IDs are required"),
        { status: 400 }
      );
    }
  ...
  async sendFileMessage({
    ...
    if (isNaN(fromId) || isNaN(recipientId)) {
      throw Object.assign(new Error("Invalid sender or recipient ID"), {
        status: 400,
      });
    }
    ...
    if (!sender || !recipient) {
      throw Object.assign(new Error("Sender or recipient not found"), {
        status: 404,
      });
    }
  ...
  async updateMessage(messageId, content, userId) {
    ...
    if (isNaN(parsedMessageId) || isNaN(parsedUserId) || !content.trim()) {
      throw Object.assign(
        new Error("Valid messageId, userId, and content are required"),
        { status: 400 }
      );
    }
  ...
  async forwardMessage(messageId, senderId, recipientIds, content) {
    ...
    if (!sender || sender.role !== "ADMIN") {
      throw Object.assign(new Error("Only admins can forward messages"), {
        status: 403,
      });
    }
    ...
    if (originalMessage.senderId !== parsedSenderId) {
      throw Object.assign(new Error("You can only forward your own messages"), {
        status: 403,
      });
    }
  ...
  async deleteMessage(messageId, userId) {
    ...
    if (isNaN(parsedMessageId) || isNaN(parsedUserId)) {
      throw Object.assign(
        new Error("Valid messageId and userId are required"),
        { status: 400 }
      );
    }
    ...
    if (!isAdmin && message.senderId !== parsedUserId) {
      throw Object.assign(new Error("You can only delete your own messages"), {
        status: 403,
      });
    }
  ...
  async getConversations(userId, role, tab) {
    try {
      const parsedUserId = parseInt(userId, 10);
      if (isNaN(parsedUserId)) {
        throw Object.assign(new Error("Valid userId is required"), {
          status: 400,
        });
      }
      if (!role || !["ADMIN", "USER"].includes(role.toUpperCase())) {
        throw Object.assign(
          new Error("Valid role (ADMIN or USER) is required"),
          { status: 400 }
        );
      }
      let conversations = await userRepository.getChatByUserId(parsedUserId);
      ...
      console.error("Error in getConversations:", error);
      throw Object.assign(
        new Error(error.message || "Failed to fetch conversations"),
        { status: 400 }
      );
    }
  },
